"""svc-streaming blueprint auto-discovery registry.

Mirrors `core/svc_presentation/blueprints/__init__.py`'s pattern:
`register_blueprints(app)` imports every submodule of this package and
collects its module-level `BLUEPRINTS: list[Blueprint]`. A new surface
group lands by dropping one new module here with its own `BLUEPRINTS`
list -- `app.py` is never edited to add one.
"""
