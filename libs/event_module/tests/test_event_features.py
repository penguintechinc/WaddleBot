"""
Event Module Feature tests
=============================

Covers :mod:`event_module.features` -- the Event Module's registration of
its two Feature contracts and their shipped default Apps against the v3
...
``event.calendar.default``'s ``permissions`` in ``event_module/features.py``
...
import pytest
from event_module.features import ScopeWideningError, register_all
from flask_core.app_registry import AppRegistry
from flask_core.feature_registry import FeatureRegistry

EXPECTED_FEATURES = {
    "event.calendar": (
        "waddles.event.calendar",
        frozenset({"event.calendar:write"}),
        "free",
    ),
    "event.ticketing": (
        "waddles.event.ticketing",
        frozenset({"event.calendar:admin"}),
        "professional",
    ),
}
...
class TestEventFeatureContracts:
    def test_both_features_registered_with_correct_tier_and_flag(
    ...
    def test_features_are_queryable_from_the_registry_by_id_and_module(
        ...
        assert feature_registry.get("event.calendar").flag == "waddles.event.calendar"
        assert {c.id for c in feature_registry.for_module("event")} == set(EXPECTED_FEATURES)


class TestEventDefaultApps:
    def test_every_feature_has_exactly_one_default_app(
        ...
        for feature_id, (flag, _, _) in EXPECTED_FEATURES.items():
            ...
            assert default_app.module == "event"
    ...
    def test_scope_widening_default_app_is_rejected(self) -> None:
        ...
        import event_module.features as features_module

        original_defs = features_module._DEFAULT_APP_DEFS
        widened_index = next(
            i
            for i, d in enumerate(original_defs)
            if d["app_id"] == "waddles.event.calendar.default"
        )
        widened = dict(original_defs[widened_index])
        widened["permissions"] = ("event.calendar:write", "event.calendar:admin")
...
class TestCalendarGate:
    ...
    The real handler (``hub_api/blueprints/v1/event.py``'s table-driven
    ``_make_handler``) has a heavy dependency chain (pydal, quart) out of
    scope for this unit suite. What's asserted here is the gate condition
    the handler guards on -- ``feature_enabled("waddles.event.calendar",
    ...
    async def test_calendar_flag_off_is_not_entitled(self) -> None:
        from event_module.features import build_contracts
        from flask_core.feature_registry import entitled_features

        contracts = build_contracts()
        calendar = next(c for c in contracts if c.id == "event.calendar")
        check = FakeCheck(enabled_flags=set())  # waddles.event.calendar OFF

        result = await entitled_features(tenant="acme", contracts=(calendar,), check=check)

        assert result == []

    async def test_calendar_flag_on_is_entitled(self) -> None:
        from event_module.features import build_contracts
        from flask_core.feature_registry import entitled_features

        contracts = build_contracts()
        calendar = next(c for c in contracts if c.id == "event.calendar")
        check = FakeCheck(enabled_flags={"waddles.event.calendar"})
