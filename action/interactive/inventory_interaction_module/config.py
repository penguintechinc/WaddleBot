...
class Config:
    ...
    MODULE_PORT = int(os.getenv('MODULE_PORT', '8024'))
    ...
    def load_credentials_from_db(cls, db_connection) -> bool:
        ...
        try:
            rows = db_connection.executesql(
                ...
                "WHERE platform = 'inventory_interaction' "
            ...
            if rows and rows[0]:
                ...
                logger.info(
                    "Credentials loaded from platform_integrations for inventory_interaction"
                )
                return True
    ...
    def start_credential_listener(cls, redis_client) -> Optional[threading.Thread]:
        ...
        channel = "credentials:inventory_interaction:bot:refreshed"
