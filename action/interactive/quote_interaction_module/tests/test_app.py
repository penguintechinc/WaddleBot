"""Tenant-isolation regression tests for quote_interaction_module's app.py.

regression: tenant-isolation audit 2026-08-30 -- ``add_quote`` (POST
/api/v1/quotes) called ``get_tenant_context`` but never applied
...
only the auth/tenant wiring (the ``waddles.social.quote`` gate check and
the 401 short-circuit before it) without needing a live ``quote_service``
or database -- ``QuoteService`` itself is out of scope here.
...
class TestAddQuoteTenantIsolation:
    ...
    async def test_no_jwt_is_rejected_not_run_as_default_tenant(
        ...
        response = await client.post(
            "/api/v1/quotes", json={"community_id": 1, "text": "hi"}
        )
