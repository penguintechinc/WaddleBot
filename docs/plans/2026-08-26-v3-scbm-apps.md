...
**Step 3b: hub-api gains three surfaces, one authorization path.** gRPC for services, REST
for browsers/CLI, MCP for AI agents — see the design doc. It is REST-only today (32 route
files, no gRPC, no MCP).

The gates go **below** the surface, shared by all three: `tenant → scope → feature`. A REST
endpoint with three gates beside an MCP tool with none is the same breach, and bolting gates
onto the transport rather than the handler is how that happens.

MCP tool definitions derive from Feature contracts rather than being hand-written, so scopes
and versioning come along and cannot drift. **Tool listing is an authorization decision**: the
visible tool set is per tenant, because Features are — a Free tenant must not be shown
Enterprise tools.

**Test:** an agent authenticated for a Free tenant lists tools and sees no Enterprise Feature;
the same agent calling that tool by name anyway is refused. Write both to fail first — a tool
hidden from listing but callable is not gated.
