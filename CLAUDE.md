...
- `POST /router/events` - Single event processing from collectors
...
- `POST /router/responses` - Submit response from interaction module or webhook
...
1. **Message Reception**: Collector receives chat message
2. **Command Detection**: Check for `!` (local container) or `#` (community module) prefix
3. **Router Forwarding**: Send to router with entity context
4. **Command Lookup**: Router queries commands table with read replica
5. **String Matching Fallback**: If no command found, check message against string patterns for:
...
6. **Permission Check**: Verify entity has command enabled (for both direct commands and triggered commands)
7. **Rate Limiting**: Check user/command/entity rate limits
8. **Execution Routing**: 
...
9. **Module Response Processing**: Interaction modules respond back to router with:
   - **Success Status**: Whether module executed properly
   - **Response Action**: chat, media, or ticker
   - **Response Data**: Content specific to action type
10. **Response Handling**: Return result to collector for user response and OBS integration
11. **Logging**: Record execution, performance metrics, usage stats, string match statistics, and module responses
...
- **Response Actions**: Support for chat, media, and ticker responses
- **Response Types**:
