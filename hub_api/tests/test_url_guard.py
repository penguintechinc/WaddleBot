...
Covers the three surfaces the security review flagged on the shared
`is_private_host`/`validate_url`/`guarded_get` half of the module: scheme
allowlist, resolved-IP rejection (literal IPs and DNS-rebind via a mocked
resolver, never real network DNS -- deterministic and offline-safe), and
redirect re-validation (`guarded_get`, exercised end-to-end with
`httpx.MockTransport` rather than a live server). Also covers the M7
Streaming port's own independent `validate_outbound_url` half -- see that
class group below.
...
`socket.getaddrinfo` is monkeypatched throughout the `validate_outbound_url`
suite too, rather than relying on real DNS -- a sandboxed/egress-filtered
CI runner (security.md: "deny-by-default outbound") may have no route to
resolve an arbitrary hostname, and a real lookup makes the suite non-
deterministic regardless. Literal-IP inputs (used elsewhere, e.g.
`tests/test_v1_music_blueprint.py`) don't need this since
`validate_outbound_url` short-circuits before ever calling the resolver
for those.
"""
