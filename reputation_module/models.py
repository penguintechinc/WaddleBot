"""
Database models for the Reputation module
...
# Reputation scoring table
reputation_scoring = db.define_table(
    'reputation_scoring',
    Field('id', 'id'),
    Field('event_name', 'string', required=True),
    Field('event_score', 'integer', required=True),
...
user_reputation = db.define_table(
    'user_reputation',
    Field('id', 'id'),
...
reputation_events = db.define_table(
    'reputation_events',
    Field('id', 'id'),
    Field('user_id', 'string', required=True),
    Field('community_id', 'reference communities', required=True),
    ...
    Field('event_name', 'string', required=True),
    Field('event_score', 'integer', required=True),
    Field('previous_score', 'integer', required=True),
    Field('new_score', 'integer', required=True),
...
GLOBAL_COMMUNITY_ID = 1  # We'll ensure this is always ID 1